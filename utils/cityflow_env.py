...
class Intersection:
    def __init__(self, inter_id, dic_traffic_env_conf, eng, light_id_dict, path_to_log, lanes_length_dict):
        ...
        self.phase_age = [0 for _ in range(self.num_phases)] 
    ...
    def wavelet_domain_denoise(self, data_signal):
    ...
    def _update_feature(self):
        ...
        dic_feature["lane_queue_vehicle_in"] = self._get_lane_queue_length_1()
        ...
        dic_feature["num_in_deg"] = self._orgnize_waiting_several_segments()  # [None, 12*4]
        # -------- reward------------------
    ...
    def _get_traffic_movement_pressure_efficient(self, enterings, exitings):
        ...
        t_m_p = [enterings[j] - outs_maps[turn_maps[j]]/3 for j in range(self.num_lane)]
        if self.padding:
    ...
    def _orgnize_waiting_several_segments(self):
        part1, part2, part3, part4 = self._get_waiting_several_segments(lane_vehicles=self.dic_lane_vehicle_current_step,
                                                                vehicle_distance=self.dic_vehicle_distance_current_step,
                                                                vehicle_speed=self.dic_vehicle_speed_current_step,
                                                                lane_length=self.lane_length,
                                                                list_lanes=self.list_lanes)
        ...
        total_in = []
        for i in range(len(self.list_entering_lanes)):
            total_in.extend([part1[i], part2[i], part3[i], part4[i]])
        if self.padding:
            total_in = total_in + self.padding1
        return total_in
 
    def _get_lane_queue_length_1(self):
        num_in_part1, num_in_part2, num_in_part3, num_in_part4 = self._get_waiting_several_segments(lane_vehicles=self.dic_lane_vehicle_current_step,
                                                                vehicle_distance=self.dic_vehicle_distance_current_step,
                                                                vehicle_speed=self.dic_vehicle_speed_current_step,
                                                                lane_length=self.lane_length,
                                                                list_lanes=self.list_lanes)
        ...
        total_in = []
        for i in range(len(self.list_entering_lanes)):
            total_in.append(sum([num_in_part1[i], num_in_part2[i], num_in_part3[i], num_in_part4[i]]))
     
        return total_in
   
    def compressive_sensing_denoise(self, noisy_matrix, Phi):
        alpha = 0.1  # 正则化参数
    ...
    def plot_matrix(self, matrix, title, ax):
        """
        Plot a sparse matrix.
        
        Parameters:
        - matrix: The matrix to plot (can be dense or sparse).
    ...
    def _get_waiting_several_segments(self, lane_vehicles, vehicle_distance, vehicle_speed,
                              lane_length, list_lanes):
        # get four segments [100, 200, 300, 400] for segment
        obs_length = self.dic_traffic_env_conf['SEGMENTS']
        inter_count = int(400/obs_length)
        ...
        seg_in_part = np.zeros((inter_count, len(self.list_entering_lanes)))
        for i in range(inter_count):
            seg_in_part[i] = np.array([len(segments[i][lane]) for lane in self.list_entering_lanes])
        original = copy.deepcopy(seg_in_part)
        
        if self.dic_traffic_env_conf['IS_DENOISE'] is False:
            if self.dic_traffic_env_conf['NOISE_TYPE'] != -1:
                noise = self.dic_traffic_env_conf['NOISE_SCALE'] * np.random.randn(seg_in_part.shape[1])
                seg_in_part += noise
        else:
            ...
            if self.dic_traffic_env_conf['NOISE_TYPE'] != -1:
                for i in range(inter_count):
                    noise = self.dic_traffic_env_conf['NOISE_SCALE'] * np.random.randn(seg_in_part.shape[0])
                    noisy_matrix[i] = Phi @ seg_in_part[i] + noise
    
        if  self.dic_traffic_env_conf['IS_DENOISE']:

            noisematrix = copy.deepcopy(noisy_matrix)
            denoised_image = self.compressive_sensing_denoise(noisy_matrix, Phi)
            ...
            # if np.sum(seg_in_part) > 10:
            #     fig, axs = plt.subplots(1, 3, figsize=(15, 5))    
            #     self.plot_matrix(original, "Original Matrix", axs[0])
            #     self.plot_matrix(noisy_matrix, "Noisy Matrix", axs[1])
            #     self.plot_matrix(denoised_image, "Denoised Matrix", axs[2])    
    ...
    def _get_waiting_several_segments_by_obs(self, lane_vehicles, vehicle_distance, vehicle_speed,
                              lane_length, list_lanes):
        # get four segments [100, 200, 300, 400] for segment
        obs_length = 8
        part1, part2, part3, part4 = {}, {}, {}, {}
    ...
    # #NEW REWARD FUNCTION
    # def update_phase_age_in_inter(self, selected_phase):
    ...
    #     dic_reward["pressure"] = np.absolute(np.sum(self.dic_feature["pressure"]))
    #     dic_reward["queue_length"] = np.absolute(np.sum(self.dic_feature["lane_queue_vehicle_in"]))
    #     # dic_reward["starvation"] = sum([age**2 for age in self.phase_age])
    #     dic_reward["starvation"] = sum([np.exp(age / 10.0) for age in self.phase_age])
    ...
    #                 reward += dic_reward_info[r] * dic_reward[r]
    #         else:
    #             print(f"Warning: Reward component {r} not found in calculations!")
...
class CityFlowEnv:
    ...
    def step(self, action1, action2, is_continous=False):
        """
        ...
        for i, inter_id in enumerate(self.list_need_action):
            ...
            #UNCOMMENT THIS FOR MY VERSION
            # self.list_intersection[inter_id].update_phase_age_in_inter(action1[i])
            # 记录 action， cur_state
    ...
    def select_phase_feature(self, state):
        ...
        return: [None, 2]
        """
        if self.dic_traffic_env_conf['MODEL_NAME'] == 'PressLightCycle':
            feat = state
        else:
