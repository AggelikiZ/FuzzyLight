...
class FuzzyLight(object):
    def __init__(self, dic_agent_conf, dic_traffic_env_conf, dic_path, cnt_round, intersection_id="0"):
        ...
        self.STARVATION_LIMIT = 5   #in decision steps
    ...
    # def update_phase_age(self, selected_phase):
    #     for p in range(self.num_phases):
    #         self.phase_age[p] += 1
    #     self.phase_age[selected_phase] = 0
    ...
    # def demand_membership(self, x, downstream_density=0):
    #     penalty = 1.0
    #     if downstream_density > 12:
    #         penalty = 0.5
    ...
    # def fuzzy_inference_engine(self, demand, starvation):
    #     d_low, d_med, d_high = self.demand_membership(demand)
    #     s_low, s_high = self.starvation_membership(starvation)

    #     flow_score = (d_high ** 2 * 15.0) + (d_med * 4.0) + (d_low * 0.1)
    #     multiplier = 1.0 + (s_high * 2.0)

    #     return flow_score * multiplier
    ...
    #     demands = [feat10, feat20, feat30, feat40]
    #     fuzzy_scores = []
        
    #     for i in range(self.num_phases):
    #         score = self.fuzzy_inference_engine(demands[i], self.phase_age[i])
    #         fuzzy_scores.append(score)

    #     scores = np.array(fuzzy_scores)
    #     total_demand = np.sum(scores)
        
    #     # Εύρεση των δύο καλύτερων φάσεων
    #     top_idx = np.argsort(scores)[-2:]
    #     dynamic_margin = 0.03 + (total_demand / 300.0)

    #     max_score = scores[top_idx[1]] if scores[top_idx[1]] > 0 else 1e-6
    #     margin = (scores[top_idx[1]] - scores[top_idx[0]]) / max_score

    #     if margin < dynamic_margin and self.current_phase is not None and self.current_phase in top_idx:
