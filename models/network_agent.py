...
class NetworkAgent(Agent):
    ...
    # def choose_action(self, count, state):
    #     """
    #     Choose action following the FuzzyLight design principle:
    #     - Phase selection is deterministic (no RL).
    #     - RL is used only to modulate the green time duration.
    #     """

    #     # -------- Stage 1: deterministic phase selection --------
    #     chosen_phase = self.phase_control_policy(state)

    #     # -------- Stage 2: duration control via RL --------
    #     state_input = self.convert_state_to_input(state)
    ...
    #         if self.action_to_phase[a] == chosen_phase
    #     ]

    #     if len(phase_q_values) == 0:
    #         return random.randrange(self.num_actions)

    #     q_phase = np.mean(phase_q_values)
    #     h_fuzzy = 1.0 / (1.0 + np.exp(-q_phase))   # in (0,1)

    #     total_vehicles = np.sum(state['lane_num_vehicle'])
    #     t_ref = np.clip(20.0 + 0.4 * total_vehicles, 20.0, 60.0)

    #     target_duration = h_fuzzy * t_ref

    #     best_action = None
    #     min_diff = float("inf")

    #     for action_id in range(self.num_actions):
    #         if self.action_to_phase[action_id] == chosen_phase:
    #             current_duration = self.action_to_duration[action_id]
    ...
    #         best_action = random.randrange(self.num_actions)
