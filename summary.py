        ...
        for round_rl in round_files:
            ...
            for inter_index in range(num_intersection):
                try:
                    ...
                    #round_dir = os.path.join(round_dir, 'generator_0')
            ...
            durations = vehicle_duration.values

            max_wait = np.max(durations)
            p95_wait = np.percentile(durations, 95)
            p99_wait = np.percentile(durations, 99)

            starved_120 = np.sum(durations > 120)
            starved_180 = np.sum(durations > 180)
...
if __name__ == "__main__":
    ...
    #memo = "benchmark_directlight"
    memo = "docker_run_jn_all"
